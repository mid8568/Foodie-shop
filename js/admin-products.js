...
function initTabs(){
    ...
    tabs.forEach(tab=>{
        ...
        tab.addEventListener("click",function(){
            ...
            filterParams.statusTab=this.innerText.split("(")[0].trim();
...
async function loadProducts(page=1){
    ...
    if(filterParams.keywordId){
        ...
        let ids=filterParams.keywordId
        .split(/[,，\s]+/)
        .map(id=>id.trim())
        .filter(id=>id);
    ...
    if(error){
        ...
        `<tr><td colspan="8" style="text-align:center;color:red;padding:20px;">商品加载失败</td></tr>`;
...
function renderProducts(products){
    ...
    if(!products.length){
        ...
        box.innerHTML=
        `<tr><td colspan="8" style="text-align:center;color:#999;padding:20px;">暂无商品</td></tr>`;
    ...
    products.forEach(item=>{
        ...
        let isOnline=item.stock_status==="上架";


        let statusClass=isOnline?"status-tag":"status-tag offline";

        let statusText=item.stock_status||"下架";
        ...
        let name=escapeHtml(item.name||"未命名商品");

        let url1688=item["1688_url"]?
        escapeHtml(item["1688_url"])
        :
        "";
...
<td width="70">
<img src="${escapeHtml(item.image)||"https://via.placeholder.com/50"}" class="table-image">
</td>
...
${url1688?`<a href="${url1688}" target="_blank">1688链接</a>`:""}
...
<a href="javascript:void(0)" onclick="toggleStatus('${item.id}','${item.stock_status}')">
${isOnline?"下架商品":"上架商品"}
</a>
...
function searchProduct(){
    ...
    filterParams.keywordTitle=document.getElementById("search-title")?.value.trim()||"";

    filterParams.keywordId=document.getElementById("search-id")?.value.trim()||"";

    filterParams.keywordCode=document.getElementById("search-code")?.value.trim()||"";
...
function resetSearch(){
    ...
    let code=document.getElementById("search-code");
...
function toggleSelectAll(masterCheckbox){
    ...
    const list=document.querySelectorAll(".select-item");
...
function renderPaginationInfo(){
    ...
    const box=document.getElementById("page-info")||document.getElementById("pagination");
...
<button class="btn-page" onclick="changePage(${currentPage-1})" ${currentPage<=1?"disabled":""}>
&lt; 上一页
</button>
...
<button class="btn-page" onclick="changePage(${currentPage+1})" ${currentPage>=totalPages?"disabled":""}>
下一页 &gt;
</button>
...
<input type="number" id="jump-page-input" value="${currentPage}" style="width:45px;height:26px;text-align:center;">

<button class="btn-page" onclick="jumpToPage()">
GO
</button>
